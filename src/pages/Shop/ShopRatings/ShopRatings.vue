...
export default {
  data() {
    return {
      onlyShowText: true,  //是否显示只看有内容的评论
      selectType: 2        //选择的评论类型:0满意 1不满意 2全部
    }
  },
  mounted() {
  ...
  computed: {
    ...
    filterRatings() {
      ...
      return ratings.filter(ratings => {
        ...
        //       text:有值/没值(空字符串)
        //        !onlyShowText 没有要求  
        //        onlyShowText   必须text.length>0 
        return (selectType === 2 || selectType === rateType) && (!onlyShowText || text.length > 0)
        // 返回 如果选择的是全部 或者选择的数字跟api中一样 并且选择的是没有要求是不是文字 或者长度大于0
        // 比如返回不满足 并且没有要求 则返回该满足的信息 
      })
    }
  },
  methods: {
